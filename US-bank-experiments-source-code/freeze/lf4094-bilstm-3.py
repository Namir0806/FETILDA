...
class BERT_Arch(nn.Module):
    ...
    def forward(self, sent_id, mask, hist):
        ...
        attn_weights.data[attn_weights.data < thresh] = 0.0

        #print("Paper atten weights: ", attn_weights)
        ...
        df_xy["x"]=[x]
        df_xy["y"]=[y]
        df_xy.to_csv('xy_lf4094_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_ep5.csv')
...
#print("Paper test_text: ", test_text)
...
config = AutoConfig.from_pretrained(bert_path + "/longformer-base-4096/", output_attentions=True) 
# import BERT-base pretrained model
bert = AutoModel.from_pretrained(bert_path + "/" + 'longformer-base-4096/', config=config) 
...
path = 'saved_weights_lf_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_ep4.pt'
model.load_state_dict(torch.load(path))

# define the optimizer
optimizer = AdamW(model.parameters(),
                  lr = 14e-4)          # learning rate
...
for epoch in range(epochs):
    ...
    torch.save(model_to_save.state_dict(), 'saved_weights_lf_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_ep5.pt')
...
path = 'saved_weights_lf_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_ep5.pt'
model.load_state_dict(torch.load(path))
...
#pearsonr = (stats.pearsonr(preds, test_y))[0]
...
mse_file = open('mse_lf4094_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm.txt', "w")
mse_file.write(mse + "\n")
mse_file.write(str(mae) + "\n")
mse_file.write(str(r2) + "\n")
#mse_file.write(str(pearsonr) + "\n")
...
test_error.to_csv('error_lf4094_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm.csv', index=False)   
'''
