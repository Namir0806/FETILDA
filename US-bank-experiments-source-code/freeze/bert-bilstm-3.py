...
class BERT_Arch(nn.Module):
    ...
    def forward(self, sent_id, mask, hist):
        ...
        #print("Paper attention weights: ", attn_weights)
        thresh = 0.1
        ...
        df_xy["x"]=[x]
        df_xy["y"]=[y]
        df_xy.to_csv('xy_bert_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_ep5.csv')
...
path = 'saved_weights_bert_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_ep4.pt' 
...
for epoch in range(epochs):
    ...
    torch.save(model_to_save.state_dict(), 'saved_weights_bert_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_ep5.pt')
...
path = 'saved_weights_bert_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_ep5.pt'
model.load_state_dict(torch.load(path))
...
mse_file = open('mse_bert_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm.txt', "w")
mse_file.write(mse+ "\n")
...
test_error.to_csv('error_bert_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm.csv', index=False) 
'''
