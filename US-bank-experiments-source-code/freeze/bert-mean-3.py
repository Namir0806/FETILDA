...
class BERT_Arch(nn.Module):
    ...
    def forward(self, sent_id, mask, hist):
        ...
        # output layer
        y = self.fc2(x) #cls_vec_)

        df_xy=pd.DataFrame()
        df_xy["x"]=[x]
        df_xy["y"]=[y]
        df_xy.to_csv('xy_bert_'+str(max_length)+'_'+sec+'_'+bv+'_mean_ep5.csv')
...
path = 'saved_weights_bert_'+str(max_length)+'_'+sec+'_'+bv+'_mean_ep4.pt'
...
for epoch in range(epochs):
    ...
    torch.save(model_to_save.state_dict(), 'saved_weights_bert_'+str(max_length)+'_'+sec+'_'+bv+'_mean_ep5.pt')
...
path = 'saved_weights_bert_'+str(max_length)+'_'+sec+'_'+bv+'_mean_ep5.pt'
model.load_state_dict(torch.load(path))
...
#print("type: ", type(preds), type(test_y), preds.dtype, test_y.dtype)
...
print("kendall: ", kendallr)

mse_file = open('mse_bert_'+str(max_length)+'_'+sec+'_'+bv+'_mean.txt', "w")
mse_file.write(mse + "\n")
...
test_error.to_csv('error_bert_'+str(max_length)+'_'+sec+'_'+bv+'_mean.csv', index=False) 
#print("before test error: ", test_error.head(2))
#test_error.to_csv("error_dp1_lr1e3_lf512_chunk_ft.csv", index=False)
#print("test error: ", test_error.head(2))
'''
