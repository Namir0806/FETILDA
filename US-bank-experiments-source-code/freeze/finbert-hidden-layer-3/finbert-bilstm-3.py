...
class BERT_Arch(nn.Module):
    ...
    def forward(self, sent_id, mask, hist):
        ...
        x =self.leakyrelu(x)
        x = self.dropout(x) 
        #look at the value for x: see if it is the same for all of them
...
for epoch in range(epochs):
    ...
    torch.save(model_to_save.state_dict(), 'saved_weights_finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm.pt')
...
path = 'saved_weights_finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm.pt'
model.load_state_dict(torch.load(path))
