...
class BERT_Arch(nn.Module):
    ...
    def __init__(self, bert, tfidf_dim):
        ...
        # dense layer 1
        self.fc1 = nn.Linear(769+tfidf_dim,int((770+tfidf_dim)/2))
      
        # dense layer 2 (Output layer)
        self.fc2 = nn.Linear(int((770+tfidf_dim)/2),1)
    ...
    def forward(self, sent_id, mask, hist, tfidf_id):
        ...
        x = prod_sum #attn_out
        x = torch.cat((x, hist.unsqueeze(0).unsqueeze(0) , tfidf_id.unsqueeze(0)), dim=1)
...
def train(epoch):
    ...
    for i in range(len(train_seq)):
        ...
        tfidf_id = torch.tensor(X_train_concat[i].tolist()).to(device)
        sent_id = train_seq[i]
        mask = train_mask[i]
        hist = train_hist[i] 
        ...
        with autocast():
            ...
            x, preds = model(sent_id, mask, hist, tfidf_id)
...
def evaluate():
    ...
    for i in range(len(valid_seq)):
        ...
        tfidf_id = torch.tensor(X_valid_concat[i].tolist()).to(device)
        sent_id = valid_seq[i]
        mask = valid_mask[i]
        hist = valid_hist[i]
...
X_train_concat = X_train_concat.toarray()
X_valid_concat = X_valid_concat.toarray()
X_test_concat = X_test_concat.toarray()
...
#X_train_concat = torch.tensor(X_train_concat.to_list()).to(device)
#X_valid_concat = torch.tensor(X_valid_concat.to_list()).to(device)
#X_test_concat = torch.tensor(X_test_concat.to_list()).to(device)
...
model = BERT_Arch(bert, X_train_concat.shape[-1])
...
for epoch in range(epochs):
    ...
    if valid_loss < best_valid_loss:
        ...
        torch.save(model_to_save.state_dict(), 'saved_weights_finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.pt')
...
valid_loss_file = open('best_valid_loss_finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.txt', 'w')
...
path = 'saved_weights_finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.pt'
...
mse_file = open('mse_finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.txt', "w")
mse_file.write(mse + "\n")
