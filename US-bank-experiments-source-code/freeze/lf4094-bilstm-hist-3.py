...
class BERT_Arch(nn.Module):
    ...
    def forward(self, sent_id, mask, hist):
        ...
        df_xy["x"]=[x]
        df_xy["y"]=[y]
        df_xy.to_csv('xy_lf4094_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_hist_ep5.csv')
...
def train():
    ...
    empty_cache_indices = [int(len(train_seq)*0.25),int(len(train_seq)*0.5),int(len(train_seq)*0.75),int(len(train_seq)-1)]
...
path = 'saved_weights_lf_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_ep4_hist.pt'
...
for epoch in range(epochs):
    ...
    torch.save(model_to_save.state_dict(), 'saved_weights_lf_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_ep5_hist.pt')
...
path = 'saved_weights_lf_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_ep5_hist.pt'
model.load_state_dict(torch.load(path))
...
mse_file = open('mse_lf4094_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_hist.txt', "w")
mse_file.write(mse + "\n")
...
test_error.to_csv('error_lf4094_'+str(max_length)+'_'+sec+'_'+bv+'_bilstm_hist.csv', index=False) 
'''
