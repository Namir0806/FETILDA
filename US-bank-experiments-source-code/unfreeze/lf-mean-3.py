...
class BERT_Arch(nn.Module):
    ...
    def forward(self, sent_id, mask, hist):
        ...
        df_xy["x"]=[x]
        df_xy["y"]=[y]
        df_xy.to_csv('xy_lf_'+str(max_length)+'_'+sec+'_'+bv+'_mean_ep5.csv')
...
path = 'saved_weights_lf_'+str(max_length)+'_'+sec+'_'+bv+'_mean_ep4.pt'
...
for epoch in range(epochs):
    ...
    torch.save(model_to_save.state_dict(), 'saved_weights_lf_'+str(max_length)+'_'+sec+'_'+bv+'_mean_ep5.pt')
...
path = 'saved_weights_lf_'+str(max_length)+'_'+sec+'_'+bv+'_mean_ep5.pt'
model.load_state_dict(torch.load(path))
...
mse_file = open('mse_lf_'+str(max_length)+'_'+sec+'_'+bv+'_mean.txt', "w")
mse_file.write(mse + "\n")
...
test_error.to_csv('error_lf_'+str(max_length)+'_'+sec+'_'+bv+'_mean.csv', index=False)  
'''
