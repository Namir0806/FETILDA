...
def train(epoch):
    ...
    memory_file = open('memory_max-2-hk-finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(epoch)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.txt', 'a+')
    model.train()
...
path = 'saved_weights_max-2-hk-finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.pt'
model.load_state_dict(torch.load(path))

# set initial loss to previous best
with open('best_valid_loss_max-2-hk-finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.txt') as f:
    lines = f.readlines()
...
for epoch in range(epochs):
    ...
    if valid_loss < best_valid_loss:
        ...
        torch.save(model_to_save.state_dict(), 'saved_weights_max-2-hk-finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.pt')
        #torch.save(model_to_save.state_dict(), 'saved_weights_max-2-hk-finbert_'+str(max_length)+'_'+sec+'_'+bv+'_epoch'+str(start_epoch+epoch)+'_of_'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.pt')
...
valid_loss_file = open('best_valid_loss_max-2-hk-finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.txt', 'w')
...
print("max-2-hk-finbert mse: ", mse)

mse_file = open('mse_max-2-hk-finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.txt', "w")
mse_file.write(mse + "\n")
...
test_error.to_csv('error_max-2-hk-finbert_'+str(max_length)+'_'+sec+'_'+bv+'_mean_hist.csv', index=False)          
