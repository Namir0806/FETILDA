...
with open('best_valid_loss_max-4-1-hk-finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.txt') as f:
    lines = f.readlines()
...
print("max-4-1-hk-finbert bare mse: "+str(test_mses[0])+'---bare---'+str(valid_mses[0]))
lr = LinearRegression()
...
mse_file = open('mse_max-4-1-hk-finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.txt', "w")
mse_file.write(mse + "\n")
