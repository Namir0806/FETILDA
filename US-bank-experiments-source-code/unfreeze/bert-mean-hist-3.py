...
class BERT_Arch(nn.Module):
    ...
    def forward(self, sent_id, mask, hist):
        ...
        df_xy["x"]=[x]
        df_xy["y"]=[y]
        df_xy.to_csv('xy_bert_'+str(max_length)+'_'+sec+'_'+bv+'_mean_hist_ep5.csv')
...
path = 'saved_weights_bert_'+str(max_length)+'_'+sec+'_'+bv+'_mean_ep4_hist.pt' 
model.load_state_dict(torch.load(path))

# define the optimizer
optimizer = AdamW(model.parameters(),
                  lr = 1e-3)          # learning rate = 5e-4 e.g. 1e-4
...
for epoch in range(epochs):
    ...
    torch.save(model_to_save.state_dict(), 'saved_weights_bert_'+str(max_length)+'_'+sec+'_'+bv+'_mean_ep5_hist.pt')
...
path = 'saved_weights_bert_'+str(max_length)+'_'+sec+'_'+bv+'_mean_ep5_hist.pt'
model.load_state_dict(torch.load(path))
...
mse_file = open('mse_bert_'+str(max_length)+'_'+sec+'_'+bv+'_mean_hist.txt', "w")
mse_file.write(mse + "\n")
