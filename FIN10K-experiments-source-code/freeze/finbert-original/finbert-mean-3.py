...
for epoch in range(epochs):
    ...
    torch.save(model_to_save.state_dict(), 'saved_weights_finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_mean.pt')
...
path = 'saved_weights_finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_mean.pt'
model.load_state_dict(torch.load(path))
...
mse_file = open('mse_finbert_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_mean.txt', "w")
mse_file.write(mse + "\n")
