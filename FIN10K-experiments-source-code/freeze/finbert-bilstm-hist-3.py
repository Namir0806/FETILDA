...
path = 'saved_weights_finbert_'+str(max_length)+'_taiwan_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.pt'
model.load_state_dict(torch.load(path))
# set initial loss to previous best
with open('best_valid_loss_finbert_'+str(max_length)+'_taiwan_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.txt') as f:
    lines = f.readlines()
