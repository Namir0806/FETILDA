...
def train(epoch):
    ...
    memory_file = open('memory_longformer_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(epoch)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm.txt', 'a+')
    model.train()
...
path = 'saved_weights_longformer_'+str(max_length)+'_taiwan_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm.pt'
model.load_state_dict(torch.load(path))

# set initial loss to previous best
with open('best_valid_loss_longformer_'+str(max_length)+'_taiwan_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm.txt') as f:
    lines = f.readlines()
...
for year in years:
    ...
    mse_file = open('mse_longformer_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm.txt', "w")
    mse_file.write(mse + "\n")
