...
path = 'saved_weights_longformer_'+str(max_length)+'_taiwan_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.pt'
model.load_state_dict(torch.load(path))
# set initial loss to previous best
with open('best_valid_loss_longformer_'+str(max_length)+'_taiwan_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.txt') as f:
    lines = f.readlines()
...
for year in years:
    ...
    print("longformer bare mse: "+str(test_mses[0])+'---bare---'+str(valid_mses[0]))
    lr = LinearRegression()
    ...
    mse_file = open('mse_longformer_'+str(max_length)+'_'+sec+'_'+bv+'_ep'+str(total_epochs)+'_lr='+'{:.1e}'.format(learning_rate)+'_bilstm_hist.txt', "w")
    mse_file.write(mse + "\n")
